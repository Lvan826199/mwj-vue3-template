...
export function getTimeState() {
    ...
    if (hours >= 6 && hours <= 10) return `⛅早上好！新的一天又开始了！`;
    if (hours >= 10 && hours <= 14) return `🌞中午好！祝你今天过得愉快！`;
    if (hours >= 14 && hours <= 18) return `🌞下午好！祝你度过愉快的一天！`;
    if (hours >= 18 && hours <= 24) return `🌛晚上好！来猛烈的开干吧！🎉🎉`;
    if (hours >= 0 && hours <= 6) return `🌚凌晨了!大兄弟!滚去睡觉吧！`;
}
...
/**
 * @description 递归深拷贝对象数据,用于需要更改时，但不改变源数据场景，如编辑或短暂编辑
 * */
